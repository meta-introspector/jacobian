...
PUBLIC_COMMANDS: tuple[CommandContract, ...] = (
    ...
    CommandContract(
        name="test-focused",
        help="Edit loop: run explicit TESTS through its semantic LANE (for example, LANE=math).",
        mutates_checkout=False,
    ...
    CommandContract(
        name="handoff-scoped",
        help="Scoped lint, types, and one owner path; not final branch validation.",
        mutates_checkout=False,
    ...
    CommandContract(
        name="affected",
        help="Final-tree validation: CI-planned affected owners and scoped static checks.",
        mutates_checkout=False,
